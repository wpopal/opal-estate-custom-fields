...
class OpalEtFieldCreator_Create_Fields {
    ...
    public function __construct(){
        ...
        add_action("wp_ajax_creator_custom_type", array( $this, 'creator_custom_type' ));
        add_action("wp_ajax_nopriv_creator_custom_type", array( $this, 'creator_custom_type' ));

        add_action("wp_ajax_create_option_select", array( $this, 'create_option_select' ));
        add_action("wp_ajax_nopriv_create_option_select", array( $this, 'create_option_select' ));
    ...
    public function create_option_select(){
        ...
        if(isset($_POST['index']) && isset($_POST['checked_default']) && isset($_POST['option_index'])){
            ...
            $result = array('type' => 'success', 'html' => $html);
        }else{
            $result = array('type' => 'fail', 'html' => '');
        }
    ...
    public function save_data(){
        ...
        if (isset( $_POST['nonce_field_save_etfields'] )) {
            ...
            if(wp_verify_nonce( $_POST['nonce_field_save_etfields'], 'action_save_etfields' )){
                ...
                $meta_key_check_exist = array();

                if($custom_fields_flag){

                    if($_POST["in_reset_default"] == 1){
                        update_option('opal_etfields_fields', array());
                    }else{
    ...
    public function admin_page_display(){
        ...
        wp_enqueue_script("creatorfields", OPALETFIELDS_PLUGIN_URL . 'assets/js/fields.js', array( 'jquery' ), "1.3", false);

        wp_enqueue_script("fonticonpicker", OPALETFIELDS_PLUGIN_URL . 'assets/fontIconPicker-2.0.0/jquery.fonticonpicker.min.js', array( 'jquery' ), "1.3", false);
        ...
        if(!$custom_fields){
            ...
            foreach( $data as $key =>  $item ){
                if(isset($item['type'])){
                    if(in_array($item['type'], array('text', 'text_date', 'textarea', 'select', 'checkbox'))){
